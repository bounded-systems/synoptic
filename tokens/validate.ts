...
import { z } from "zod";
import { ColorPair, Dimension, Measure, PrimitiveColor, PropertyPair, PropertyToken, RootFontSize } from "./schema.ts";
...
{ const dim = JSON.parse(Deno.readTextFileSync(new URL("examples/dimensions.derived.json", import.meta.url))); const r = RootFontSize.safeParse(dim.dimension.$root); const m = Measure.safeParse(dim.dimension.$measure); console.log(`  ${r.success ? "✓" : "✗"} $root (RootFontSize + rem-floor)   ${m.success ? "✓" : "✗"} $measure (ch, ≤80)`); allOk = r.success && m.success && allOk; }

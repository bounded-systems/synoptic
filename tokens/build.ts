...
import { deriveContrastPairs, derivePrimitivePairs, derivePrimitives, derivePropertyTokens } from "./verbs.ts";

const paletteFile = Deno.args[0];
if (!paletteFile) { console.error("usage: build.ts <palette.json> [outDir]"); Deno.exit(1); }
...
console.log(`wrote 4 token files → ${outDir} (${palette.length} colors → ${pp.$leafCount} valid pairs, merkle ${pp.$merkleRoot.slice(0, 12)}…)`);

...
import { deriveContrastPairs, deriveDimensions, deriveMeasure, derivePrimitivePairs, derivePrimitives, derivePropertyTokens, deriveRoot } from "./verbs.ts";
...
write("dimensions.derived.json", {
  dimension: { $description: "Derived dimensions — rem atoms LOCKED to rem (px/em/ch unconstructable: they fail Resize 1.4.4 / Reflow 1.4.10), on a geometric (Weber-JND) step. $root is the fluid clamp() every rem floats on; $measure is the ch reading-width token (≤80ch, 1.4.8). AAA floors noted per atom (2.75rem target 2.5.5, 0.125rem focus 2.4.11).", $root: deriveRoot(true), $measure: deriveMeasure(), ...deriveDimensions(scale) },
});
